...
function ResultSearchPeople(props) {
  ...
  async function sendRequestHandler(e) {
    ...
    requestSpan.current.innerText=requestSpan.current.innerText==="+"?"Sending...":"Canceling..."
    const result=await sendRequestAuth(`${BACKEND_BASEURL}api/FriendRequest`,method,{
  ...
  const peopleJsx = props.people?.map((p) => {
    return (
      <li
        ...
        onClick={async e=>{await sendRequestHandler(e)}}
      >
